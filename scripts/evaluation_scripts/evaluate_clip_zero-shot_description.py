...
# 🔧 Corrige descrições que vierem como listas
    for i in range(len(class_texts)):
        if isinstance(class_texts[i], list):
            # Junta frases em uma única string
            class_texts[i] = ". ".join(class_texts[i])

    # Exibe um exemplo já convertido (para debug)
    example_text = class_texts[0]
    if isinstance(example_text, str):
        print(f"   Exemplo de description: {example_text[:100]}...")
    else:
        print(f"   Exemplo de description: [tipo {type(example_text)}]")
